...
import { INVOICE_CATEGORY_CONFIG, INVOICE_STATUS_CONFIG } from "@/config/invoices";
...
export function InvoiceForm({
  ...
  const effectiveSettings = useMemo(
    () => getEffectiveInvoiceSettings(INVOICE_SETTINGS, branchId),
    [branchId],
  );
  ...
  const [invoiceNumber, setInvoiceNumber] = useState(
    invoice?.invoiceNumber ??
      buildInvoiceNumber(
        effectiveSettings?.invoicePrefix ?? "INV",
  ...
  const [taxRate, setTaxRate] = useState(
    String(invoice?.taxRate ?? effectiveSettings?.defaultTaxRate ?? 0),
  );
  ...
  const [note, setNote] = useState(invoice?.note ?? effectiveSettings?.defaultNote ?? "");
  ...
  useEffect(() => {
    ...
    const settings = getEffectiveInvoiceSettings(INVOICE_SETTINGS, branchId);
    ...
    setDueDate(addDays(issueDate, settings.paymentTermDays));
  ...
  function updateLineItem(
    id: string,
    field: keyof Pick<InvoiceLineItem, "description" | "quantity" | "unitPrice">,
    value: string,
  ) {
    setLineItems((currentItems) =>
      currentItems.map((item) =>
        item.id === id
          ? {
              ...item,
              [field]: field === "description" ? value : Math.max(Number(value) || 0, 0),
            }
          : item,
      ),
    );
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    onSave({
      ...
      balanceAmount: Math.max(totals.totalAmount - currentPaidAmount, 0),
      paymentMethod: invoice?.paymentMethod,
      paymentReference: invoice?.paymentReference,
      paidAt: invoice?.paidAt,
      createdBy: invoice?.createdBy ?? CURRENT_ADMIN.name,
      note: note.trim(),
    });
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-7">
      <section className="grid gap-5 sm:grid-cols-2">
        <FormField
          ...
          error={
            submitted && !invoiceNumber.trim() ? "Enter an invoice number" : undefined
          }
        >
          <Input
            id="invoiceNumber"
        ...
        <FormField label="Category" htmlFor="invoiceCategory">
          <Select
            id="invoiceCategory"
            value={category}
        ...
        <FormField label="Invoice status" htmlFor="invoiceStatus">
          <Select
            id="invoiceStatus"
            value={status}
        ...
        <FormField label="Issue date" htmlFor="invoiceIssueDate">
          <Input
            id="invoiceIssueDate"
            type="date"
            value={issueDate}
      ...
      <section>
        ...
        <div className="mt-5 grid gap-5 sm:grid-cols-2">
          ...
          <FormField label="Client phone" htmlFor="invoiceClientPhone" optional>
            <Input
          ...
          <FormField label="Billing address" htmlFor="invoiceClientAddress" optional>
            <Input
          ...
          {lineItems.map((item, index) => (
            <div key={item.id} className="rounded-control border border-border p-4">
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-[minmax(0,1fr)_8rem_10rem]">
                ...
                <FormField label="Quantity" htmlFor={`invoiceItemQuantity-${item.id}`}>
                  <Input
                ...
                <FormField label="Unit price" htmlFor={`invoiceItemPrice-${item.id}`}>
                  <Input
      ...
      <section>
        <h3 className="font-bold">Totals and tax</h3>

        <div className="mt-5 grid gap-5 sm:grid-cols-2">
          <FormField label="Tax rate" htmlFor="invoiceTaxRate">
            <Input
              id="invoiceTaxRate"
              type="number"
              min="0"
          ...
          <FormField label="Discount amount" htmlFor="invoiceDiscountAmount">
            <Input
        ...
        <dl className="mt-5 grid gap-3 sm:grid-cols-2">
          <div className="rounded-control bg-canvas p-4">
            ...
            <dd className="mt-1 font-bold">{formatPKR(totals.subtotal)}</dd>
          </div>

          <div className="rounded-control bg-canvas p-4">
            <dt className="text-xs text-text-muted">Tax</dt>

            <dd className="mt-1 font-bold">{formatPKR(totals.taxAmount)}</dd>
          </div>

          <div className="rounded-control bg-canvas p-4">
            <dt className="text-xs text-text-muted">Discount</dt>

            <dd className="mt-1 font-bold">{formatPKR(totals.discountAmount)}</dd>
          </div>
      ...
      <FormField label="Invoice note" htmlFor="invoiceNote" optional>
        <Textarea
          id="invoiceNote"
          value={note}
