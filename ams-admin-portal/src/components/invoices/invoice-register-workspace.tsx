...
import { exportInvoicesToCsv } from "@/lib/invoices";
...
export function InvoiceRegisterWorkspace() {
  ...
  const visibleInvoices = useMemo(() => {
    ...
    return scopedInvoices.filter((invoice) => {
      ...
      return searchableValue.includes(query) && matchesStatus && matchesCategory;
    });
  ...
  return (
    <div className="mx-auto max-w-360">
      <PageHeader
        ...
        description={INVOICE_COPY.register.description}
        actions={
          <>
            <Button
              variant="outline"
              onClick={() => exportInvoicesToCsv(visibleInvoices)}
            >
              <Download />
      ...
      <Card className="mt-6 overflow-hidden">
        <div className="border-b border-border p-5">
          <h2 className="text-lg font-bold">{INVOICE_COPY.register.registerTitle}</h2>

          <p className="mt-1 text-sm text-text-muted">
            {INVOICE_COPY.register.registerDescription}
          </p>

          <SearchFilterBar
            searchValue={searchQuery}
            onSearchChange={setSearchQuery}
            searchPlaceholder={INVOICE_COPY.common.searchPlaceholder}
            ...
            filters={[
              {
                ...
                children: (
                  <>
                    <option value="all">{INVOICE_COPY.common.allCategories}</option>
              ...
              },
              {
                ...
                children: (
                  <>
                    <option value="all">{INVOICE_COPY.common.allStatuses}</option>
        ...
        footer={
          selectedInvoice ? (
            <div className="flex flex-wrap justify-end gap-3">
              <Button variant="outline" onClick={() => duplicateAndOpen(selectedInvoice)}>
                <Copy />
      ...
      <Drawer
        ...
        title={editorMode === "create" ? "Create invoice" : "Edit invoice"}
        description="Create or update client billing using the effective branch settings."
      >
        {editorMode && (
          <InvoiceForm
            key={editorMode === "create" ? "new-invoice" : selectedInvoice?.id}
            invoice={editorMode === "edit" ? (selectedInvoice ?? undefined) : undefined}
