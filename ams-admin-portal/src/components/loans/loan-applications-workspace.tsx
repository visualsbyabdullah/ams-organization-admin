...
import { LOAN_STATUS_CONFIG, LOAN_TYPE_CONFIG } from "@/config/loans";
...
const APPLICATION_STATUSES = ["pending_approval", "approved", "rejected"] as const;
...
export function LoanApplicationsWorkspace() {
      ...
      loans.filter(
        (loan) =>
          APPLICATION_STATUSES.includes(
            loan.status as (typeof APPLICATION_STATUSES)[number],
          ) &&
          (selectedBranch.isAggregate || loan.branchId === selectedBranch.id),
      ),
  ...
  const visibleApplications = useMemo(() => {
    ...
    return applicationLoans.filter((loan) => {
  ...
  const pendingApplications = applicationLoans.filter(
    (loan) => loan.status === "pending_approval",
  );

  const approvedApplications = applicationLoans.filter(
    (loan) => loan.status === "approved",
  );

  const rejectedApplications = applicationLoans.filter(
    (loan) => loan.status === "rejected",
  );

  const pendingAmount = pendingApplications.reduce(
    (total, loan) => total + loan.requestedAmount,
    0,
  );

  const approvedAmount = approvedApplications.reduce(
    (total, loan) => total + loan.approvedAmount,
    0,
  );
  ...
  const metrics = [
    ...
    },
    {
      ...
      value: formatPKR(pendingAmount, true),
    ...
    },
    {
      ...
      value: formatPKR(approvedAmount, true),
      detail: "Ready for disbursement",
      icon: CheckCircle2,
    ...
    setLoans((currentLoans) =>
      currentLoans.map((loan) => {
        ...
        const installmentAmount = Math.ceil(
          values.approvedAmount / values.installmentCount,
        );
        ...
        return {
          ...
          installmentAmount,
          paidAmount: 0,
          outstandingBalance: values.approvedAmount,
          repaymentStartDate: values.repaymentStartDate,
          nextDueDate: values.repaymentStartDate,
  ...
  function rejectLoan(loanId: string, note: string) {
    setLoans((currentLoans) =>
      currentLoans.map((loan) =>
        loan.id === loanId
          ? {
              ...
              reviewedBy: CURRENT_ADMIN.name,
              note: note || "Application rejected during review.",
            }
          : loan,
      ),
    );
  ...
  function reopenLoan(loanId: string) {
    setLoans((currentLoans) =>
      currentLoans.map((loan) =>
        loan.id === loanId
          ? {
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <Card className="order-2 h-fit p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-text-muted">
                Oldest pending applications requiring review.
              </p>
            </div>
          </div>
          ...
          <div className="mt-5 space-y-3">
            {pendingApplications.length > 0 ? (
              [...pendingApplications]
                .sort((first, second) =>
                  first.requestDate.localeCompare(second.requestDate),
                )
                .map((loan) => {
                  const employee = EMPLOYEES.find((item) => item.id === loan.employeeId);
              ...
              <div className="rounded-control bg-success-muted p-4 text-sm font-medium text-success">
                No loan applications are waiting for approval.
              </div>
            )}
          </div>
        </Card>
        ...
        <Card className="order-1">
          <div className="border-b border-border p-5">
            <h2 className="text-lg font-bold">Application register</h2>
          ...
          {visibleApplications.length > 0 ? (
                  ...
                  {visibleApplications.map((loan) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell>
                          <strong>{formatPKR(loan.requestedAmount)}</strong>
                        </TableCell>

                        <TableCell>{loan.installmentCount}</TableCell>
            ...
            <div className="flex min-h-72 flex-col items-center justify-center p-8 text-center">
              ...
              <h3 className="mt-4 font-bold">No loan applications found</h3>

              <p className="mt-2 text-sm text-text-muted">
                Change the filters or submit a new employee loan request.
              </p>
            </div>
          )}
        </Card>
