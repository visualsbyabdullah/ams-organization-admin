...
export function LoanRepaymentsWorkspace() {
  ...
  const [repayments, setRepayments] = useState<LoanRepayment[]>(LOAN_REPAYMENTS);
  ...
  const scopedRepayments = useMemo(
    () =>
      repayments.filter(
        (repayment) =>
          selectedBranch.isAggregate || repayment.branchId === selectedBranch.id,
      ),
    [repayments, selectedBranch],
  );
  ...
  const visibleRepayments = useMemo(() => {
    ...
    return scopedRepayments.filter((repayment) => {
      const employee = EMPLOYEES.find((item) => item.id === repayment.employeeId);

      const loan = EMPLOYEE_LOANS.find((item) => item.id === repayment.loanId);
      ...
      const matchesSource = sourceFilter === "all" || repayment.source === sourceFilter;

      const matchesPeriod =
        periodFilter === "all" || repayment.payrollPeriod === periodFilter;

      return (
        searchableValue.includes(query) && matchesStatus && matchesSource && matchesPeriod
      );
    });
  ...
  const selectedLoan = selectedRepayment
    ? EMPLOYEE_LOANS.find((loan) => loan.id === selectedRepayment.loanId)
    : null;

  const currentPeriodRepayments = scopedRepayments.filter(
    (repayment) => repayment.payrollPeriod === LOAN_REPAYMENT_REFERENCE_PERIOD,
  );

  const scheduledAmount = currentPeriodRepayments
    .filter((repayment) => !["waived", "failed"].includes(repayment.status))
    .reduce((total, repayment) => total + repayment.amount, 0);

  const collectedAmount = currentPeriodRepayments.reduce(
    (total, repayment) => total + repayment.paidAmount,
    0,
  );

  const overdueAmount = scopedRepayments
    .filter((repayment) => repayment.status === "overdue")
    .reduce((total, repayment) => total + repayment.balanceAmount, 0);

  const collectionRate =
    scheduledAmount > 0 ? Math.round((collectedAmount / scheduledAmount) * 100) : 0;

  const collectionQueue = scopedRepayments
    .filter((repayment) =>
      ["due", "partial", "overdue", "failed"].includes(repayment.status),
    )
  ...
  const metrics = [
    {
      ...
      value: formatPKR(scheduledAmount, true),
      detail: "July 2026",
      icon: WalletCards,
      tone: "info" as const,
    },
  ...
  function recordPayment(repaymentId: string, values: LoanRepaymentPaymentValues) {
    setRepayments((currentRepayments) =>
      currentRepayments.map((repayment) => {
        ...
        const nextPaidAmount = Math.min(
          repayment.paidAmount + values.paidAmount,
          repayment.amount,
        );

        const nextBalance = Math.max(repayment.amount - nextPaidAmount, 0);
        ...
        return {
          ...
          processedBy: CURRENT_ADMIN.name,
          note: values.note,
        };
      }),
    );
  ...
  function processPayrollDeductions() {
    ...
    setRepayments((currentRepayments) =>
      currentRepayments.map((repayment) => {
        ...
        const shouldProcess =
          inScope &&
          repayment.payrollPeriod === LOAN_REPAYMENT_REFERENCE_PERIOD &&
          repayment.source === "payroll_deduction" &&
          ["due", "partial", "overdue"].includes(repayment.status);
        ...
        return {
          ...
          processedBy: CURRENT_ADMIN.name,
          note: repayment.note || "Collected through the payroll deduction batch.",
        };
      }),
    );
  }

  function updateStatus(repaymentId: string, status: LoanRepaymentStatus) {
    setRepayments((currentRepayments) =>
      currentRepayments.map((repayment) =>
        repayment.id === repaymentId
          ? {
              ...
              processedBy:
                status === "waived" ? CURRENT_ADMIN.name : repayment.processedBy,
              note:
                status === "waived"
                  ? repayment.note || `Installment waived by ${CURRENT_ADMIN.name}.`
                  : repayment.note,
            }
          : repayment,
      ),
  ...
  return (
    <div className="mx-auto max-w-360">
      <PageHeader
        eyebrow={LOAN_REPAYMENTS_COPY.eyebrow}
        title={LOAN_REPAYMENTS_COPY.title}
        description={LOAN_REPAYMENTS_COPY.description}
        actions={
          <>
            ...
            <Button onClick={processPayrollDeductions}>
              <Banknote />
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <Card className="order-1 overflow-hidden">
          <div className="border-b border-border p-5">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
              ...
              <div className="w-full shrink-0 xl:w-52">
                ...
                >
                  {LOAN_REPAYMENT_PERIOD_OPTIONS.map((option) => (
          ...
          {visibleRepayments.length > 0 ? (
                ...
                <TableBody>
                  {visibleRepayments.map((repayment) => {
                    const employee = EMPLOYEES.find(
                      (item) => item.id === repayment.employeeId,
                    );

                    const loan = EMPLOYEE_LOANS.find(
                      (item) => item.id === repayment.loanId,
                    );
                    ...
                    const statusConfig = LOAN_REPAYMENT_STATUS_CONFIG[repayment.status];
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell>{formatPeriod(repayment.payrollPeriod)}</TableCell>

                        <TableCell>{formatDate(repayment.dueDate)}</TableCell>

                        <TableCell>{formatPKR(repayment.amount)}</TableCell>

                        <TableCell>{formatPKR(repayment.paidAmount)}</TableCell>

                        <TableCell>
                          <strong>{formatPKR(repayment.balanceAmount)}</strong>
                        </TableCell>
            ...
            <div className="flex min-h-72 flex-col items-center justify-center p-8 text-center">
              ...
              <h3 className="mt-4 font-bold">{LOAN_REPAYMENTS_COPY.emptyTitle}</h3>
        ...
        <Card className="order-2 h-fit p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold">{LOAN_REPAYMENTS_COPY.attentionTitle}</h2>
          ...
          <div className="mt-5 space-y-3">
            {collectionQueue.length > 0 ? (
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <Badge
                        variant={
                          LOAN_REPAYMENT_STATUS_CONFIG[repayment.status].badgeVariant
                        }
                      >
                        {LOAN_REPAYMENT_STATUS_CONFIG[repayment.status].label}
                      </Badge>
                    </div>
              ...
              <div className="rounded-control bg-success-muted p-4 text-sm font-medium text-success">
                All current loan installments have been collected.
              </div>
            )}
          </div>
        </Card>
        ...
        footer={
          selectedRepayment ? (
            <div className="flex flex-wrap justify-end gap-3">
              {selectedRepayment.status === "paid" && (
                <Button
                  variant="outline"
                  onClick={() => updateStatus(selectedRepayment.id, "due")}
                >
                  <RotateCcw />
                  Reverse payment
                </Button>
              ...
              {["due", "partial", "overdue", "failed"].includes(
                selectedRepayment.status,
              ) && (
                <>
                  <Button
                    variant="outline"
                    onClick={() => updateStatus(selectedRepayment.id, "waived")}
                  >
                    <X />
                    Waive balance
                  </Button>
              ...
              {selectedRepayment.status === "waived" && (
                <Button
                  variant="outline"
                  onClick={() => updateStatus(selectedRepayment.id, "due")}
                >
      ...
      >
        {selectedRepayment && selectedEmployee && selectedLoan && (
          <div className="space-y-6">
            <section className="rounded-card border border-border">
              <div className="flex items-start justify-between gap-4 border-b border-border p-5">
                <div>
                  <h3 className="font-bold">
                    {LOAN_TYPE_CONFIG[selectedLoan.type].label} · Installment #
                ...
                <Badge
                  variant={
                    LOAN_REPAYMENT_STATUS_CONFIG[selectedRepayment.status].badgeVariant
                  }
                >
                  {LOAN_REPAYMENT_STATUS_CONFIG[selectedRepayment.status].label}
                </Badge>
              </div>
            ...
            <section>
              <h3 className="text-sm font-bold">Collection progress</h3>

              <div className="mt-3 grid gap-3 sm:grid-cols-3">
                <div className="rounded-control bg-canvas p-4">
                  <p className="text-xs text-text-muted">Scheduled</p>

                  <p className="mt-1 font-bold">{formatPKR(selectedRepayment.amount)}</p>
                </div>
                ...
                <div className="rounded-control bg-success-muted p-4">
                  ...
                  <p className="mt-1 font-bold text-success">
                    {formatPKR(selectedRepayment.paidAmount)}
                  </p>
                </div>

                <div className="rounded-control bg-warning-muted p-4">
                  <p className="text-xs text-warning">Balance</p>
            ...
            <section>
              <h3 className="text-sm font-bold">Repayment note</h3>

              <p className="mt-2 rounded-control bg-canvas p-4 text-sm leading-6 text-text-muted">
                {selectedRepayment.note || "No repayment note has been added."}
              </p>
            </section>
          </div>
        )}
      ...
      <Drawer
        open={Boolean(paymentOpen && selectedRepayment)}
