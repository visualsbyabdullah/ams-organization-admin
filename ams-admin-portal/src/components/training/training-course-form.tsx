...
export function TrainingCourseForm({
  ...
  const [scope, setScope] = useState<TrainingCourseScope>(
    course?.scope ?? "organization",
  );

  const [branchId, setBranchId] = useState(
    course?.branchId ?? (selectedBranchId === "all" ? "" : selectedBranchId),
  );

  const [status, setStatus] = useState<TrainingCourseStatus>(course?.status ?? "draft");

  const [deliveryMode, setDeliveryMode] = useState<TrainingDeliveryMode>(
    course?.deliveryMode ?? "virtual",
  );

  const [durationHours, setDurationHours] = useState(String(course?.durationHours ?? 2));

  const [passingScore, setPassingScore] = useState(String(course?.passingScore ?? 70));

  const [mandatory, setMandatory] = useState(course?.mandatory ?? false);

  const [certificationValidityMonths, setCertificationValidityMonths] = useState(
    String(course?.certificationValidityMonths ?? 0),
  );

  const [capacity, setCapacity] = useState(String(course?.capacity ?? 30));

  const [provider, setProvider] = useState(course?.provider ?? "");

  const [ownerName, setOwnerName] = useState(course?.ownerName ?? CURRENT_ADMIN.name);
  ...
  const durationValue = Math.max(Number(durationHours) || 0, 0);

  const passingScoreValue = Math.min(Math.max(Number(passingScore) || 0, 0), 100);

  const capacityValue = Math.max(Number(capacity) || 0, 0);

  const validityValue = Math.max(Number(certificationValidityMonths) || 0, 0);
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    onSave({
      ...
      mandatory,
      certificationValidityMonths: validityValue,
    ...
    <form onSubmit={handleSubmit} className="space-y-7">
      <section className="grid gap-5 sm:grid-cols-2">
        <FormField
          ...
          error={submitted && !title.trim() ? "Enter a course title" : undefined}
        >
          <Input
        ...
        <FormField
          ...
          error={submitted && !code.trim() ? "Enter a course code" : undefined}
        >
          <Input
        ...
        <FormField label="Category" htmlFor="trainingCourseCategory">
          <Select
        ...
        <FormField label="Course status" htmlFor="trainingCourseStatus">
          <Select
            id="trainingCourseStatus"
            value={status}
            onChange={(event) => setStatus(event.target.value as TrainingCourseStatus)}
          >
        ...
        <FormField label="Course scope" htmlFor="trainingCourseScope">
          <Select
            id="trainingCourseScope"
            value={scope}
            onChange={(event) => setScope(event.target.value as TrainingCourseScope)}
          >
            {Object.entries(TRAINING_COURSE_SCOPE_CONFIG).map(([value, config]) => (
              <option key={value} value={value}>
                {config.label}
              </option>
            ))}
        ...
        <FormField label="Delivery mode" htmlFor="trainingDeliveryMode">
          <Select
        ...
        <FormField
          ...
          error={submitted && durationValue <= 0 ? "Enter a valid duration" : undefined}
        >
          <Input
        ...
        >
          <Input
            ...
            onChange={(event) => setCertificationValidityMonths(event.target.value)}
          />
        </FormField>
        ...
        <FormField
          ...
          error={submitted && !provider.trim() ? "Enter a provider" : undefined}
        >
          <Input
            id="trainingProvider"
            value={provider}
        ...
        <FormField
          ...
          error={submitted && !ownerName.trim() ? "Enter a course owner" : undefined}
        >
          <Input
            id="trainingOwner"
            value={ownerName}
      ...
      <div className="flex items-center justify-between gap-5 rounded-control border border-border p-4">
        <div>
          <p className="text-sm font-semibold">Mandatory training</p>
      ...
      <FormField
        ...
        error={
          submitted && !description.trim() ? "Enter a course description" : undefined
        }
      >
        <Textarea
      ...
      <FormField label="Internal note" htmlFor="trainingCourseNote" optional>
        <Textarea
