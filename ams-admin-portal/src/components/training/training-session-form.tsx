...
export function TrainingSessionForm({
  ...
  const [sessionDate, setSessionDate] =
    useState(
      session?.sessionDate ?? "2026-08-01",
    );
  ...
  const [capacity, setCapacity] = useState(
    String(session?.capacity ?? 30),
  );

  const [enrolledCount, setEnrolledCount] =
    useState(String(session?.enrolledCount ?? 0));
  ...
  const enrolledValue = Math.min(
    Math.max(Number(enrolledCount) || 0, 0),
    capacityValue,
  );
    ...
    >
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <FormField
          ...
          error={
            submitted && !venue.trim()
              ? "Enter a venue or meeting platform"
              : undefined
          }
        >
          <Input
        ...
        <FormField
          ...
          htmlFor="trainingSessionEnrollments"
        >
          <Input
            ...
            value={enrolledCount}
            onChange={(event) =>
              setEnrolledCount(event.target.value)
            }
          />
        </FormField>
      </div>
