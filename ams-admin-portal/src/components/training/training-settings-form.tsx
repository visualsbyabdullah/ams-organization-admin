...
export function TrainingSettingsForm({
  ...
  const [scope, setScope] =
    useState<TrainingSettingsScope>(
  ...
  const [status, setStatus] =
    useState<TrainingSettingsStatus>(
      settings?.status ?? "draft",
    );

  const [defaultDueDays, setDefaultDueDays] =
    useState(String(settings?.defaultDueDays ?? 30));
  ...
  const [allowSelfEnrollment, setAllowSelfEnrollment] =
    useState(settings?.allowSelfEnrollment ?? true);
  ...
  const [autoIssueCertificates, setAutoIssueCertificates] =
    useState(
      settings?.autoIssueCertificates ?? true,
    );

  const [certificationExpiryReminderDays, setCertificationExpiryReminderDays] =
    useState(
      String(
        settings?.certificationExpiryReminderDays ?? 30,
      ),
    );

  const [mandatoryCompletionTarget, setMandatoryCompletionTarget] =
    useState(
  ...
  const dueDaysValue = Math.max(
    Number(defaultDueDays) || 0,
    1,
  );

  const reminderValue = Math.max(
    Number(reminderDaysBeforeDue) || 0,
    0,
  );

  const expiryReminderValue = Math.max(
    Number(certificationExpiryReminderDays) || 0,
    0,
  );

  const completionTargetValue = Math.min(
    Math.max(
      Number(mandatoryCompletionTarget) || 0,
      0,
    ),
    100,
  );
  ...
  function handleSubmit(
    ...
    onSave({
      ...
      certificationExpiryReminderDays:
        expiryReminderValue,
      mandatoryCompletionTarget:
  ...
  const controls = [
    {
      ...
      description:
        "Employees may request enrollment in published optional courses.",
    ...
    },
    {
      ...
      description:
        "Self-enrollment requests require manager approval before assignment.",
  ...
  return (
    ...
    >
      <section className="grid gap-5 sm:grid-cols-2">
        ...
        <FormField
          label="Status"
          htmlFor="trainingSettingsStatus"
        >
          <Select
            ...
            onChange={(event) =>
              setStatus(
                event.target.value as TrainingSettingsStatus,
              )
            }
          >
            {Object.entries(
        ...
        >
          <Select
            ...
            onChange={(event) =>
              setScope(
                event.target.value as TrainingSettingsScope,
              )
            }
          >
            {Object.entries(
        ...
        <div className="mt-5 grid gap-5 sm:grid-cols-2">
          ...
          >
            <Input
              ...
              onChange={(event) =>
                setDefaultDueDays(event.target.value)
              }
            />
          </FormField>
          ...
          >
            <Input
              ...
              onChange={(event) =>
                setCertificationExpiryReminderDays(
                  event.target.value,
                )
              }
            />
      ...
      <section>
        <h3 className="font-bold">
          Enrollment and certification controls
        </h3>
