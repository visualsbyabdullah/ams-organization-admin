...
export function CompensationForm({
  ...
  const [
    payFrequency,
    setPayFrequency,
  ] = useState<PayFrequency>(
    record?.payFrequency ??
      "monthly",
  );

  const [status, setStatus] =
    useState<CompensationStatus>(
      record?.status ??
        "pending_review",
    );
  ...
  const [
    ...
    useState<CompensationChangeReason>(
      record?.lastChangeReason ??
        "annual_review",
    );

  const [note, setNote] =
    useState(record?.note ?? "");
    ...
    >
      ...
      <section>
        ...
        <p className="mt-1 text-sm text-text-muted">
          Configure recurring compensation
          components paid with base salary.
        </p>
        ...
        <div className="mt-4 rounded-control bg-success-muted p-4">
          ...
          <p className="mt-1 text-xl font-bold text-success">
            PKR{" "}
            {totalFixedCompensation.toLocaleString(
              "en-PK",
            )}
          </p>
        </div>
      ...
      <section className="grid gap-5 md:grid-cols-2">
        ...
        >
          <Select
            ...
            onChange={(event) =>
              setChangeReason(
                event.target
                  .value as CompensationChangeReason,
              )
            }
          >
            {Object.entries(
        ...
        <FormField
          ...
          htmlFor="compensationEffectiveDate"
        >
          <Input
