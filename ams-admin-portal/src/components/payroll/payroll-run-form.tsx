...
export function PayrollRunForm({
  ...
  const [branchId, setBranchId] = useState(
    selectedBranchId === "all" ? "all" : selectedBranchId,
  );
  ...
  const branchOptions = useMemo(() => {
    ...
    EMPLOYEES.forEach((employee) => {
      uniqueBranches.set(employee.branchId, employee.branchName);
    });
  ...
  const isValid = Boolean(period && branchId && payDate);
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const periodConfig = getPeriodDates(period);
    ...
    const employeeCount =
      branchId === "all"
        ? EMPLOYEES.length
        : EMPLOYEES.filter((employee) => employee.branchId === branchId).length;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <FormField
        ...
        error={submitted && !period ? "Select a payroll period" : undefined}
      >
        <Input
          id="payrollPeriod"
          type="month"
      ...
      <FormField label="Organization scope" htmlFor="payrollBranch">
        <Select
          id="payrollBranch"
          value={branchId}
      ...
      <FormField label="Expected pay date" htmlFor="payrollPayDate">
        <Input
          id="payrollPayDate"
          type="date"
          value={payDate}
      ...
      <FormField label="Payroll note" htmlFor="payrollRunNote" optional>
        <Textarea
          id="payrollRunNote"
          value={note}
