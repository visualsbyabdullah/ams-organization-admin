...
export function StatutoryFilingForm({
  ...
  const [period, setPeriod] =
    useState(
      filing?.period ?? "2026-07",
    );

  const [category, setCategory] =
    useState<StatutoryFilingCategory>(
      filing?.category ?? "income_tax",
    );

  const [amount, setAmount] =
    useState(
      String(filing?.amount ?? 0),
    );

  const [dueDate, setDueDate] =
    useState(
      filing?.dueDate ?? "2026-08-15",
    );

  const [status, setStatus] =
    useState<StatutoryFilingStatus>(
      filing?.status ?? "draft",
    );
        ...
        >
          <Select
            ...
            onChange={(event) =>
              setCategory(
                event.target
                  .value as StatutoryFilingCategory,
              )
            }
          >
            {Object.entries(
        ...
        >
          <Select
            ...
            onChange={(event) =>
              setStatus(
                event.target
                  .value as StatutoryFilingStatus,
              )
            }
          >
            {Object.entries(
