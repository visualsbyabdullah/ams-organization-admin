...
import { PAYROLL_RUN_STATUS_CONFIG } from "@/config/payroll";
import { PAYROLL_RUN_PERIOD_OPTIONS, PAYROLL_RUNS_COPY } from "@/config/payroll-runs";
...
function getBranchShortName(branchName: string) {
  return branchName.replace(" Branch", "");
}
...
export function PayrollRunsWorkspace() {
  ...
  const visibleRuns = useMemo(() => {
    ...
    return scopedRuns.filter((run) => {
      const searchableValue = [run.periodLabel, run.branchName, run.createdBy, run.note]
        .join(" ")
        .toLowerCase();
      ...
      const matchesPeriod = periodFilter === "all" || run.period === periodFilter;
  ...
  const chartData = useMemo<PayrollBranchComparisonPoint[]>(
    () =>
      scopedRuns
        .filter((run) => run.period === chartPeriod)
        .filter((run) => run.branchId !== "all")
        .map((run) => ({
          branch: getBranchShortName(run.branchName),
          gross: run.grossAmount,
          deductions: run.deductionAmount,
          net: run.netAmount,
        })),
    [chartPeriod, scopedRuns],
  );

  const currentPeriodRuns = scopedRuns.filter((run) => run.period === "2026-07");

  const totalNetAmount = currentPeriodRuns.reduce(
    (total, run) => total + run.netAmount,
    0,
  );
  ...
  const metrics = [
    {
      ...
      value: currentPeriodRuns.length,
    ...
    },
    {
      ...
      value: currentPeriodRuns.filter((run) => run.status === "pending_approval").length,
      detail: "Waiting for final review",
      icon: Clock3,
    ...
    },
    {
      ...
      detail: "Completed review stage",
      icon: ShieldCheck,
      tone: "success" as const,
    },
    {
      label: "Current net payroll",
      value: formatPKR(totalNetAmount, true),
      detail: "Selected organization scope",
      icon: WalletCards,
      tone: "success" as const,
    },
  ];
  ...
  function duplicateRun(run: PayrollRun) {
  ...
  return (
    <div className="mx-auto max-w-360">
      <PageHeader
        eyebrow={PAYROLL_RUNS_COPY.eyebrow}
        title={PAYROLL_RUNS_COPY.title}
        description={PAYROLL_RUNS_COPY.description}
        actions={
          <>
      ...
      <section className="mt-6">
        <ChartCard
          title={PAYROLL_RUNS_COPY.chartTitle}
          description={PAYROLL_RUNS_COPY.chartDescription}
        >
          {chartData.length > 0 ? (
            <PayrollRunsBranchChart data={chartData} />
          ) : (
            <div className="flex h-72 items-center justify-center text-sm text-text-muted">
              No branch payroll data is available for this period.
            </div>
          )}
        </ChartCard>
      </section>
      ...
      <Card className="mt-6">
        <div className="border-b border-border p-5">
          <h2 className="text-lg font-bold">{PAYROLL_RUNS_COPY.tableTitle}</h2>
          ...
          <SearchFilterBar
            ...
            searchPlaceholder={PAYROLL_RUNS_COPY.searchPlaceholder}
            gridClassName="xl:grid-cols-[minmax(0,1fr)_14rem_14rem]"
            filters={[
              {
                value: periodFilter,
                onChange: setPeriodFilter,
                children: PAYROLL_RUN_PERIOD_OPTIONS.map((option) => (
              ...
              },
              {
                ...
                children: (
                  <>
                    <option value="all">{PAYROLL_RUNS_COPY.allStatuses}</option>
                    {Object.entries(PAYROLL_RUN_STATUS_CONFIG).map(([value, config]) => (
        ...
        {visibleRuns.length > 0 ? (
              ...
              {visibleRuns.map((run) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell>{formatPKR(run.grossAmount)}</TableCell>

                    <TableCell>{formatPKR(run.deductionAmount)}</TableCell>

                    <TableCell>
                      <strong>{formatPKR(run.netAmount)}</strong>
                    </TableCell>
          ...
          <div className="flex min-h-72 flex-col items-center justify-center p-8 text-center">
            ...
            <h3 className="mt-4 font-bold">{PAYROLL_RUNS_COPY.emptyTitle}</h3>
        ...
        footer={
          selectedRun ? (
            <div className="flex flex-wrap justify-end gap-3">
              <Button variant="outline" onClick={() => duplicateRun(selectedRun)}>
                <Copy />
                Duplicate
              </Button>

              {selectedRun.status !== "draft" && selectedRun.status !== "paid" && (
                <Button
                  variant="outline"
                  onClick={() => updateRunStatus(selectedRun.id, "draft")}
                >
                  <RotateCcw />
                  Return to draft
                </Button>
      ...
      >
        {selectedRun && (
          <div className="space-y-6">
            ...
            <section>
              <h3 className="text-sm font-bold">Processing note</h3>

              <p className="mt-2 rounded-control bg-canvas p-4 text-sm leading-6 text-text-muted">
                {selectedRun.note || "No payroll processing note has been added."}
              </p>
            </section>
            ...
            {selectedRun.status === "paid" && (
              <div className="flex items-center gap-3 rounded-control bg-success-muted p-4 text-success">
                ...
                <p className="text-sm font-semibold">
                  This payroll run has been completed and marked as paid.
                </p>
              </div>
            )}
          </div>
