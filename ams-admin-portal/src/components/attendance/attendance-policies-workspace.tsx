...
import type { AttendancePolicy } from "@/types/attendance-policy";
...
export function AttendancePoliciesWorkspace() {
  ...
  const [policies, setPolicies] = useState<AttendancePolicy[]>(ATTENDANCE_POLICIES);
  ...
  const visiblePolicies = useMemo(() => {
    ...
    return scopedPolicies.filter((policy) => {
      ...
      const searchableValue = [policy.name, policy.description, ...branchLabels]
        .join(" ")
        .toLowerCase();
  ...
  const metrics = [
    ...
    },
    {
      ...
      value: scopedPolicies.filter((policy) => policy.scope === "selected_branches")
        .length,
      detail: "Limited organization scope",
      icon: Building2,
  ...
  function savePolicy(nextPolicy: AttendancePolicy) {
  ...
  function duplicatePolicy(policy: AttendancePolicy) {
  ...
  return (
    <div className="mx-auto max-w-360">
      <PageHeader
        eyebrow={ATTENDANCE_POLICY_COPY.eyebrow}
        title={ATTENDANCE_POLICY_COPY.title}
        description={ATTENDANCE_POLICY_COPY.description}
        actions={
          <Button
            onClick={() => {
      ...
      <Card className="mt-6">
        <div className="border-b border-border p-5">
          <h2 className="text-lg font-bold">{ATTENDANCE_POLICY_COPY.policiesTitle}</h2>
          ...
          <SearchFilterBar
            ...
            searchPlaceholder={ATTENDANCE_POLICY_COPY.searchPlaceholder}
            gridClassName="lg:grid-cols-[minmax(0,1fr)_14rem_14rem]"
            filters={[
              {
                value: statusFilter,
                ...
                children: (
                  <>
                    <option value="all">{ATTENDANCE_POLICY_COPY.allStatuses}</option>
                    {Object.entries(ATTENDANCE_POLICY_STATUS_CONFIG).map(
                      ([value, config]) => (
              ...
              },
              {
                ...
                children: (
                  <>
                    <option value="all">{ATTENDANCE_POLICY_COPY.allScopes}</option>
                    {Object.entries(ATTENDANCE_POLICY_SCOPE_CONFIG).map(
                      ([value, config]) => (
                        <option key={value} value={value}>
                          {config.label}
                        </option>
        ...
        {visiblePolicies.length > 0 ? (
          <div className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-3">
            {visiblePolicies.map((policy) => {
              const statusConfig = ATTENDANCE_POLICY_STATUS_CONFIG[policy.status];

              const scopeConfig = ATTENDANCE_POLICY_SCOPE_CONFIG[policy.scope];
                    ...
                    <div className="mt-5 grid grid-cols-2 gap-3">
                      ...
                      <div className="rounded-control bg-canvas p-3">
                        ...
                        <p className="mt-1 font-bold">
                          {policy.rules.gracePeriodMinutes} min
                        </p>
                      </div>
                    </div>
          ...
          <div className="flex min-h-72 flex-col items-center justify-center p-8 text-center">
            ...
            <h3 className="mt-4 font-bold">{ATTENDANCE_POLICY_COPY.emptyTitle}</h3>
          ...
          <div className="space-y-6">
            <section className="rounded-card border border-border">
              <div className="flex items-start justify-between gap-4 border-b border-border p-5">
                ...
                <Badge
                  variant={
                    ATTENDANCE_POLICY_STATUS_CONFIG[selectedPolicy.status].badgeVariant
                  }
                >
                  {ATTENDANCE_POLICY_STATUS_CONFIG[selectedPolicy.status].label}
                </Badge>
              </div>
                ...
                items={[
                  ...
                  },
                  {
                    ...
                    value: formatMinutesAsHours(
                      selectedPolicy.rules.minimumHalfDayMinutes,
                    ),
                  },
                  {
                    label: "Standard workday",
                    value: formatMinutesAsHours(
                      selectedPolicy.rules.standardDailyMinutes,
                    ),
                  },
                  {
                    label: "Overtime begins",
                    value: formatMinutesAsHours(
                      selectedPolicy.rules.overtimeAfterMinutes,
                    ),
                  },
                  {
              ...
              <ToggleDetailList
                items={[
                  ...
                  },
                  {
                    ...
                    enabled: selectedPolicy.rules.allowRemoteAttendance,
                  },
                  {
      ...
      <Drawer
        ...
        title={
          editorMode === "create" ? "Create attendance policy" : "Edit attendance policy"
        }
      ...
      >
        {editorMode && (
          <AttendancePolicyForm
            key={editorMode === "create" ? "new-policy" : selectedPolicy?.id}
