...
export function AttendanceRecordForm({
  ...
  const [date, setDate] = useState(record?.date ?? "2026-07-16");

  const [scheduledStart, setScheduledStart] = useState(record?.scheduledStart ?? "09:00");

  const [scheduledEnd, setScheduledEnd] = useState(record?.scheduledEnd ?? "18:00");
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <FormField
        ...
        error={submitted && !date ? "Select a date" : undefined}
      >
        <Input
          id="attendanceDate"
          type="date"
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <FormField label="Check in" htmlFor="checkIn" optional>
          <Input
            id="checkIn"
            type="time"
            value={checkIn}
        ...
        <FormField label="Check out" htmlFor="checkOut" optional>
          <Input
            id="checkOut"
            type="time"
            value={checkOut}
      ...
      <FormField label="Attendance status" htmlFor="attendanceStatus">
        <Select
          id="attendanceStatus"
          value={status}
      ...
      <FormField label="Administrator note" htmlFor="attendanceNote" optional>
        <Textarea
          id="attendanceNote"
          value={note}
      ...
      <div className="flex justify-end gap-3 border-t border-border pt-5">
        ...
        <Button type="submit">{record ? "Save changes" : "Add attendance record"}</Button>
      </div>
    </form>
  );
}
