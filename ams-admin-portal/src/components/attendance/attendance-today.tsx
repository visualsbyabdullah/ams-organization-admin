...
import { ATTENDANCE_COPY, ATTENDANCE_STATUS_CONFIG } from "@/config/attendance";
...
export function AttendanceToday() {
  ...
  }, [records, searchQuery, selectedBranch, statusFilter]);

  const summary = ATTENDANCE_SUMMARIES[selectedBranchId] ?? ATTENDANCE_SUMMARIES.all;

  const trend = ATTENDANCE_TRENDS[selectedBranchId] ?? ATTENDANCE_TRENDS.all;

  const departmentData =
    DEPARTMENT_ATTENDANCE[selectedBranchId] ?? DEPARTMENT_ATTENDANCE.all;

  const exceptionRecords = visibleRecords.filter(
    (record) =>
      record.status === "late" ||
  ...
  const metrics = [
    ...
    },
    {
      ...
      detail: `${Math.round(
        (summary.present / summary.scheduled) * 100,
      )}% attendance rate`,
      icon: UserCheck,
  ...
  return (
    <div className="mx-auto max-w-360">
      <PageHeader
        eyebrow={ATTENDANCE_COPY.eyebrow}
        title={ATTENDANCE_COPY.title}
        description={ATTENDANCE_COPY.description}
        actions={
          <>
      ...
      <div className="mt-5 flex items-center gap-2 text-sm text-text-muted">
        ...
        <span>
          ...
          <strong className="text-text">{formatAttendanceDate("2026-07-16")}</strong>
        </span>
      </div>
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[1.25fr_0.75fr]">
        <ChartCard
          title={ATTENDANCE_COPY.attendanceTrend.title}
          description={ATTENDANCE_COPY.attendanceTrend.description}
        >
        ...
        <Card className="p-6">
          <div>
            <h2 className="text-lg font-bold">{ATTENDANCE_COPY.exceptions.title}</h2>

            <p className="mt-1 text-sm text-text-muted">
              {ATTENDANCE_COPY.exceptions.description}
            </p>
          </div>
          ...
          <div className="mt-6 space-y-3">
            {exceptionRecords.length > 0 ? (
              ...
              <div className="rounded-control bg-success-muted p-4 text-sm text-success">
                No attendance exceptions in the current view.
              </div>
            )}
          </div>
        </Card>
      ...
      <section className="mt-6">
        <ChartCard
          title={ATTENDANCE_COPY.departmentChart.title}
          description={ATTENDANCE_COPY.departmentChart.description}
        >
      ...
      <Card className="mt-6">
        <div className="flex flex-col gap-3 border-b border-border p-4 lg:flex-row">
          <div className="relative flex-1">
            ...
            <Input
              ...
              placeholder={ATTENDANCE_COPY.searchPlaceholder}
              className="pl-9"
            />
          </div>
          ...
          <div className="lg:w-56">
            ...
            >
              <option value="all">{ATTENDANCE_COPY.allStatuses}</option>
        ...
        {visibleRecords.length > 0 ? (
          <Table>
            <TableHeader>
              <TableRow className="bg-canvas">
                <TableHead>{ATTENDANCE_COPY.table.employee}</TableHead>

                <TableHead>{ATTENDANCE_COPY.table.schedule}</TableHead>

                <TableHead>{ATTENDANCE_COPY.table.checkIn}</TableHead>

                <TableHead>{ATTENDANCE_COPY.table.checkOut}</TableHead>

                <TableHead>{ATTENDANCE_COPY.table.worked}</TableHead>

                <TableHead>{ATTENDANCE_COPY.table.status}</TableHead>

                <TableHead className="w-16">{ATTENDANCE_COPY.table.actions}</TableHead>
              </TableRow>
            </TableHeader>
          ...
          <div className="flex min-h-72 flex-col items-center justify-center p-8 text-center">
            ...
            <h2 className="mt-4 font-bold">No attendance records found</h2>

            <p className="mt-2 text-sm text-text-muted">
              Change the filters or add an attendance record.
            </p>
          </div>
        )}
      </Card>
