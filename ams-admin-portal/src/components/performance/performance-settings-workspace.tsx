...
export function PerformanceSettingsWorkspace() {
    ...
    return scoped.filter(
      (item) =>
        [
          item.name,
          item.branchName,
          PERFORMANCE_REVIEW_FREQUENCY_CONFIG[item.reviewFrequency].label,
        ]
          .filter(Boolean)
          .join(" ")
          .toLowerCase()
  ...
  const metrics = [
    ...
    },
    {
      ...
      value: effective
        ? PERFORMANCE_REVIEW_FREQUENCY_CONFIG[effective.reviewFrequency].label
        : "â€”",
      detail: "Effective cadence",
      icon: Gauge,
      tone: "warning" as const,
    },
    {
      label: "Rating scale",
      value: effective ? `1â€“${effective.ratingScaleMaximum}` : "â€”",
      detail: "Effective scoring model",
      icon: CheckCircle2,
  ...
  return (
    <div className="mx-auto max-w-360">
      <PageHeader
        ...
        title={PERFORMANCE_COPY.settings.title}
        description={PERFORMANCE_COPY.settings.description}
        actions={
          <Button
            onClick={() => {
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_24rem]">
        <Card className="order-1 overflow-hidden">
          <div className="border-b border-border p-5">
            <h2 className="text-lg font-bold">
              {PERFORMANCE_COPY.settings.registerTitle}
            </h2>
            <p className="mt-1 text-sm text-text-muted">
              {PERFORMANCE_COPY.settings.registerDescription}
            </p>
            <SearchFilterBar
              searchValue={searchQuery}
              onSearchChange={setSearchQuery}
              searchPlaceholder="Search configuration, branch or frequency"
              ...
              filters={[
                {
                  ...
                  children: (
                    <>
                      <option value="all">All configuration scopes</option>
                      {Object.entries(PERFORMANCE_SETTINGS_SCOPE_CONFIG).map(
                        ([value, config]) => (
                ...
                },
                {
                  ...
                  children: (
                    <>
                      <option value="all">All configuration statuses</option>
                      {Object.entries(PERFORMANCE_SETTINGS_STATUS_CONFIG).map(
                        ([value, config]) => (
                          <option key={value} value={value}>
                            {config.label}
                          </option>
          ...
          <DataTable
            ...
            emptyState={
              <div className="flex min-h-72 flex-col items-center justify-center p-8 text-center">
                ...
                <h3 className="mt-4 font-bold">{PERFORMANCE_COPY.settings.emptyTitle}</h3>
                <p className="mt-2 text-sm text-text-muted">
                  {PERFORMANCE_COPY.settings.emptyDescription}
                </p>
              </div>
            }
          />
        ...
        <Card className="order-2 h-fit p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold">
                {PERFORMANCE_COPY.settings.effectiveTitle}
              </h2>
              <p className="mt-1 text-sm text-text-muted">
                {PERFORMANCE_COPY.settings.effectiveDescription}
              </p>
            </div>
          </div>
          {effective ? (
            <div className="mt-5 space-y-3">
              <div className="rounded-control border border-border p-4">
                <p className="font-bold">{effective.name}</p>
                <p className="mt-1 text-xs text-text-muted">
                  {effective.scope === "branch"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="rounded-control bg-canvas p-4">
                  ...
                  <p className="mt-1 font-bold">1â€“{effective.ratingScaleMaximum}</p>
                </div>
                <div className="rounded-control bg-canvas p-4">
                  <p className="text-xs text-text-muted">Goals</p>
                  <p className="mt-1 font-bold">{effective.goalWeight}%</p>
                </div>
                <div className="rounded-control bg-canvas p-4">
                  <p className="text-xs text-text-muted">Competencies</p>
                  <p className="mt-1 font-bold">{effective.competencyWeight}%</p>
                </div>
              </div>
              <div className="space-y-2 pt-2">
                {[
                  ["Self review", effective.requireSelfReview],
                  ["Manager review", effective.requireManagerReview],
                  ["Calibration", effective.requireCalibration],
                  ["Peer feedback", effective.allowPeerFeedback],
                ].map(([label, enabled]) => (
                  <div
                    key={String(label)}
            ...
            <p className="mt-5 rounded-control bg-warning-muted p-4 text-sm font-medium text-warning">
              No active performance configuration is available.
            </p>
          )}
        </Card>
      </section>
          ...
          selected ? (
            <div className="flex flex-wrap justify-end gap-3">
              <Button variant="outline" onClick={() => setEditorMode("edit")}>
                {PERFORMANCE_COPY.actions.editSettings}
              </Button>
              {selected.status !== "active" && (
                <Button onClick={() => updateStatus("active")}>
                  {PERFORMANCE_COPY.actions.activate}
                </Button>
              )}
              {selected.status === "active" && (
      ...
      <Drawer
        ...
        description="Configure review cadence, scoring weights and workflow controls."
      >
        {editorMode && (
          <PerformanceSettingsForm
            key={editorMode === "create" ? "new-settings" : selected?.id}
            settings={editorMode === "edit" ? (selected ?? undefined) : undefined}
