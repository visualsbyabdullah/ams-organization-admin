...
type BulkEmployeeImportDrawerProps = {
  ...
  onImported?: (employees: ImportedEmployee[]) => void;
};
...
function downloadTemplate() {
  const csv = [
    ...
    ...TEMPLATE_ROWS.map((row) => row.map(escapeCsvValue).join(",")),
  ].join("\n");
  ...
  anchor.href = url;
  anchor.download = "ams-employee-import-template.csv";
  anchor.click();
...
function parseCsv(content: string) {
  ...
  function valueAt(row: string[], header: string) {
  ...
  return lines.slice(1).map((line, index) => {
    ...
    return {
      rowNumber: index + 2,
      employeeId: valueAt(row, "employee_id"),
      firstName: valueAt(row, "first_name"),
      lastName: valueAt(row, "last_name"),
      workEmail: valueAt(row, "work_email"),
      phone: valueAt(row, "phone"),
      branch: valueAt(row, "branch"),
      department: valueAt(row, "department"),
      designation: valueAt(row, "designation"),
      lineManager: valueAt(row, "line_manager"),
      employmentType: valueAt(row, "employment_type"),
      joiningDate: valueAt(row, "joining_date"),
      basicSalary: valueAt(row, "basic_salary"),
...
function validateRows(rows: CsvRow[], selectedBranchId: string) {
  const existingCodes = new Set(
    EMPLOYEES.map((employee) => employee.employeeCode.toLowerCase()),
  );
  ...
  return rows.map((row): ValidatedRow => {
    const errors: string[] = [];
    const employeeCode = row.employeeId.trim();
    ...
    if (
      row.basicSalary &&
      (!Number.isFinite(Number(row.basicSalary)) || Number(row.basicSalary) < 0)
    ) {
      errors.push("Basic salary must be a valid amount");
    }
    ...
    if (email && (existingEmails.has(email) || seenEmails.has(email))) {
      errors.push("Duplicate work email");
    }
    ...
    const branch =
      branches.find(
        (item) =>
          item.id.toLowerCase() === row.branch.trim().toLowerCase() ||
          item.name.toLowerCase() === row.branch.trim().toLowerCase(),
      ) ?? branches.find((item) => item.id === selectedBranchId);

    if (!branch) {
      errors.push("Branch must match an AMS branch");
    }

    const employmentType = normalizeValue(row.employmentType || "full-time");
    ...
    const salary = Number(row.basicSalary || 0);

    const employee = branch
      ? ({
          id: employeeCode || crypto.randomUUID(),
          employeeCode: employeeCode || `AMS-${Date.now()}`,
          name,
          firstName: row.firstName.trim(),
          lastName: row.lastName.trim(),
          initials: createInitials(row.firstName, row.lastName),
          email,
          workEmail: email,
          ...
          managerName: row.lineManager.trim() || "Not assigned",
          lineManager: row.lineManager.trim() || "Not assigned",
          employmentType,
          joiningDate: row.joiningDate.trim(),
          joinDate: row.joiningDate.trim(),
          basicSalary: salary,
          salary,
          status,
          avatarUrl: "",
...
export function BulkEmployeeImportDrawer({
  ...
  const validRows = useMemo(
    () => rows.filter((row) => row.errors.length === 0 && row.employee),
    [rows],
  );
  ...
  function readFile(file: File | undefined) {
    ...
    reader.onload = () => {
      try {
        ...
        setRows(validateRows(parsed, selectedBranchId));
    ...
    reader.onerror = () => {
      setFileError("CSV could not be read.");
    };
  ...
  function importEmployees() {
    const employees = validRows
      .map((row) => row.employee)
      .filter((employee): employee is ImportedEmployee => Boolean(employee));

    if (employees.length === 0) {
      return;
    }

    persistImportedEmployees(employees);
    onImported?.(employees);
    setCompletedCount(employees.length);
  }
      ...
      footer={
        completedCount === null ? (
          <div className="flex flex-wrap justify-end gap-3">
            ...
            <Button onClick={importEmployees} disabled={validRows.length === 0}>
              <Upload />
              Import{" "}
              {validRows.length > 0 ? `${validRows.length} employees` : "employees"}
            </Button>
          </div>
        ) : (
      ...
      <div className="space-y-6">
        {completedCount !== null ? (
          <div className="flex min-h-80 flex-col items-center justify-center rounded-card border border-border p-8 text-center">
            ...
            <h3 className="mt-5 text-lg font-bold">Employees imported</h3>

            <p className="mt-2 max-w-sm text-sm leading-6 text-text-muted">
              {completedCount} employee
              {completedCount === 1 ? "" : "s"} were added to the People directory.
            </p>
          </div>
        ) : (
          <>
            <div className="flex flex-wrap items-center justify-between gap-4 rounded-control border border-border p-4">
              <div>
                <p className="text-sm font-semibold">Use the AMS CSV template</p>
                <p className="mt-1 text-xs text-text-muted">
                  Keep the column names unchanged before uploading.
                </p>
              </div>
            ...
            >
              ...
              <p className="mt-4 font-semibold">Drop employee CSV here</p>

              <p className="mt-2 text-sm text-text-muted">or browse from your computer</p>
              ...
              {fileName && (
                <p className="mt-4 text-xs font-semibold text-text-muted">{fileName}</p>
              )}
            </div>
              ...
              <>
                <div className="grid gap-3 sm:grid-cols-3">
                  <div className="rounded-control bg-canvas p-4">
                    <p className="text-xs text-text-muted">Uploaded rows</p>
                  ...
                  <div className="rounded-control bg-success-muted p-4 text-success">
                    ...
                    <p className="mt-2 text-2xl font-bold">{validRows.length}</p>
                  </div>

                  <div className="rounded-control bg-danger-muted p-4 text-danger">
                    <p className="text-xs">Errors</p>
                    <p className="mt-2 text-2xl font-bold">{invalidRows.length}</p>
                  </div>
                </div>
                ...
                <div className="overflow-hidden rounded-card border border-border">
                  <div className="border-b border-border p-4">
                    ...
                    <p className="mt-1 text-xs text-text-muted">
                      Invalid rows are skipped. Correct the CSV and upload it again to
                      include them.
                    </p>
                  </div>
                  ...
                  <div className="max-h-96 overflow-auto">
                    <Table>
                      ...
                      <TableBody>
                        {rows.map((row) => (
                          <TableRow key={`${row.rowNumber}-${row.employeeId}`}>
                            ...
                            <TableCell>
                              ...
                              <p className="mt-1 text-xs text-text-muted">
                                {row.employeeId || "Missing ID"} Â·{" "}
                                {row.workEmail || "Missing email"}
                              </p>
                            </TableCell>

                            <TableCell>{row.branch || "â€”"}</TableCell>
