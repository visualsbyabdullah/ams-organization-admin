...
import type { PermissionAction } from "@/types/access-control";
...
export function PermissionMatrix({
  ...
  function togglePermission(moduleId: string, action: PermissionAction) {
    ...
    const nextModulePermissions = modulePermissions.includes(action)
        ...
        {PERMISSION_MODULES.map((module) => {
          ...
          return (
            ...
            >
              ...
              {PERMISSION_ACTIONS.map((action) => {
                const checked = (permissions[module.id] ?? []).includes(action.id);
