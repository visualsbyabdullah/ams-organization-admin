...
function getDepartmentEmployeeCount(
  ...
  if (branchId === "all") {
    return Object.values(branchCounts).reduce(
...
export function OrganizationStructure() {
  ...
  const scopedEmployeeCount =
    selectedBranch.isAggregate
      ...
      : BRANCH_STRUCTURE_SUMMARIES.find(
          (branch) =>
            branch.branchId ===
            selectedBranch.id,
        )?.employeeCount ?? 0;

  const scopedManagerCount =
    selectedBranch.isAggregate
      ? BRANCH_STRUCTURE_SUMMARIES.reduce(
          (total, branch) =>
            total + branch.managerCount,
          0,
        )
      : BRANCH_STRUCTURE_SUMMARIES.find(
          (branch) =>
  ...
  const visibleBranchSummaries =
    BRANCH_STRUCTURE_SUMMARIES.filter(
      (summary) =>
        selectedBranch.isAggregate ||
        summary.branchId === selectedBranch.id,
    );
  ...
  return (
    <div className="mx-auto max-w-360">
      <PageHeader
        eyebrow={
          ORGANIZATION_STRUCTURE_COPY.eyebrow
        }
        title={
          ORGANIZATION_STRUCTURE_COPY.title
        }
        description={
          ORGANIZATION_STRUCTURE_COPY.description
        }
        actions={
          <>
        ...
        <Card className="p-6">
          <div>
            ...
            <p className="mt-1 text-sm text-text-muted">
              Workforce distribution across the
              current organization scope.
            </p>
          </div>

          <div className="mt-6 space-y-3">
            {visibleBranchSummaries.map(
              (summary) => {
                const branch =
                  BRANCH_OPTIONS.find(
                    (item) =>
          ...
          <div className="border-b border-border p-4">
            <div className="relative">
              ...
              <Input
                ...
                placeholder={
                  ORGANIZATION_STRUCTURE_COPY.departmentSearch
                }
                className="pl-9"
              />
            </div>
      ...
      <Card className="mt-6">
        <div className="flex flex-col gap-4 border-b border-border p-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-text-muted">
              Job titles and levels available to
              employees.
            </p>
          </div>
          ...
          <div className="relative w-full md:max-w-sm">
            ...
            <Input
              ...
              placeholder={
                ORGANIZATION_STRUCTURE_COPY.designationSearch
              }
              className="pl-9"
            />
          </div>
        ...
        <div className="flex items-center gap-2 border-t border-border px-5 py-4 text-xs text-text-muted">
          <ShieldCheck size={15} />
          Changes to departments and designations
