...
import { Download, Files, Search, Upload } from "lucide-react";
...
import { DOCUMENT_RECORDS, DOCUMENT_SETTINGS } from "@/data/documents";
import { EMPLOYEES } from "@/data/employees";
import { documentIsInScope, downloadCsv, resolveDocumentSettings } from "@/lib/documents";
  ...
  const visibleDocuments = useMemo(() => {
    ...
    return scopedDocuments.filter((document) => {
      ...
      return (
        ...
        (ownerFilter === "all" || document.ownerType === ownerFilter)
      );
    });
  ...
  return (
    <div className="mx-auto max-w-360">
      <PageHeader
        ...
        title={DOCUMENTS_COPY.library.title}
        description={DOCUMENTS_COPY.library.description}
        actions={
          <>
      ...
      <Card className="mt-6 overflow-hidden">
        <div className="border-b border-border p-5">
          <h2 className="text-lg font-bold">{DOCUMENTS_COPY.library.registerTitle}</h2>
          <p className="mt-1 text-sm text-text-muted">
            {DOCUMENTS_COPY.library.registerDescription}
          </p>
          ...
          <div className="mt-5 grid gap-3 xl:grid-cols-[minmax(0,1fr)_13rem_13rem_13rem]">
            <div className="relative">
              ...
              <Input
                ...
                placeholder={DOCUMENTS_COPY.library.searchPlaceholder}
                className="pl-9"
              />
            </div>
            ...
            >
              <option value="all">{DOCUMENTS_COPY.library.allCategories}</option>
            ...
            >
              <option value="all">{DOCUMENTS_COPY.library.allOwners}</option>
            ...
            >
              <option value="all">{DOCUMENTS_COPY.library.allStatuses}</option>
        ...
        <DataTable
          ...
          emptyState={
            <EmptyState
              icon={Files}
              title={DOCUMENTS_COPY.library.emptyTitle}
              description={DOCUMENTS_COPY.library.emptyDescription}
              action={
                <Button onClick={() => setUploadOpen(true)}>
                  <Upload />
                  {DOCUMENT_ACTION_LABELS.upload}
