...
import { resolveDocumentSettings, settingsAreInScope } from "@/lib/documents";
import type { DocumentSettings, DocumentSettingsStatus } from "@/types/document";
...
export function DocumentSettingsWorkspace() {
  ...
  const [settings, setSettings] = useState<DocumentSettings[]>(DOCUMENT_SETTINGS);
  ...
  const scopedSettings = useMemo(
    () => settings.filter((item) => settingsAreInScope(item, selectedBranchId)),
  ...
  const metrics = [
    {
      ...
      value: String(scopedSettings.filter((item) => item.status === "active").length),
    ...
    },
    {
      ...
      value: String(scopedSettings.filter((item) => item.scope === "branch").length),
      detail: "Custom document controls",
      icon: Building2,
      tone: "info" as const,
    },
    {
      label: "Upload limit",
      value: effectiveSettings ? `${effectiveSettings.maximumUploadMb} MB` : "Not set",
      detail: "Effective maximum file size",
      icon: HardDrive,
      tone: "info" as const,
    },
    {
      ...
      value: effectiveSettings
        ...
        : "Not set",
      detail: "Effective first reminder",
      icon: ShieldCheck,
  ...
  function duplicateSettings(source: DocumentSettings) {
  ...
  return (
    <div className="mx-auto max-w-360">
      <PageHeader
        ...
        title={DOCUMENTS_COPY.settings.title}
        description={DOCUMENTS_COPY.settings.description}
        actions={
          <Button
            onClick={() => {
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_24rem]">
        <Card className="order-1 overflow-hidden">
          <div className="border-b border-border p-5">
            <h2 className="text-lg font-bold">{DOCUMENTS_COPY.settings.registerTitle}</h2>
          ...
          <DataTable
            ...
            emptyState={
              <EmptyState
                icon={FileSearch}
                title={DOCUMENTS_COPY.settings.emptyTitle}
                description={DOCUMENTS_COPY.settings.emptyDescription}
                action={
        ...
        <Card className="order-2 h-fit p-5">
          <div className="flex items-start gap-3">
            <IconContainer icon={CheckCircle2} tone="success" />
            <div>
              <h2 className="text-lg font-bold">
                {DOCUMENTS_COPY.settings.effectiveTitle}
              </h2>
              <p className="mt-1 text-sm text-text-muted">
                {DOCUMENTS_COPY.settings.effectiveDescription}
              </p>
            </div>
          </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-control bg-canvas p-4">
                  ...
                  <p className="mt-1 text-sm font-bold">
                    {effectiveSettings.retentionYears} years
                  </p>
                </div>

                <div className="rounded-control bg-canvas p-4">
                  <p className="text-xs text-text-muted">Upload limit</p>
                  <p className="mt-1 text-sm font-bold">
                ...
                <div className="rounded-control bg-canvas p-4">
                  ...
                  <p className="mt-1 text-sm font-bold">
                    {
                      DOCUMENT_VISIBILITY_CONFIG[effectiveSettings.defaultVisibility]
                        .label
                    }
                  </p>
                </div>
      ...
      >
        {selectedSettings && <DocumentSettingsDetails settings={selectedSettings} />}
      </Drawer>
      ...
      <Drawer
        ...
        title={
          editorMode === "create" ? "Add document settings" : "Edit document settings"
        }
        description="Configure retention, expiry, upload, verification and access controls."
      >
        {editorMode && (
          <DocumentSettingsForm
            key={editorMode === "create" ? "new-document-settings" : selectedSettings?.id}
