...
import { DOCUMENT_CATEGORY_CONFIG } from "@/config/documents";
import { CURRENT_ADMIN } from "@/data/current-admin";
import { EMPLOYEES } from "@/data/employees";
import type { DocumentCategory, DocumentRequest } from "@/types/document";

type DocumentRequestFormProps = {
  selectedBranchId: string;
  onCancel: () => void;
  onCreate: (request: DocumentRequest) => void;
};
...
export function DocumentRequestForm({
  ...
  const isValid = Boolean(employeeId && title.trim() && dueDate);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-7">
      <section className="grid gap-5 sm:grid-cols-2">
        ...
        <FormField label="Category" htmlFor="requestCategory">
          <Select
            id="requestCategory"
            value={category}
        ...
        <FormField
          ...
          error={submitted && !title.trim() ? "Enter a request title" : undefined}
        >
          <Input
            id="requestTitle"
            value={title}
      ...
      <div className="flex items-center justify-between gap-5 rounded-control border border-border p-4">
        <div>
          <p className="text-sm font-semibold">Mandatory request</p>
      ...
      <FormField label="Request note" htmlFor="requestNote" optional>
        <Textarea
          id="requestNote"
          value={note}
