...
export function DocumentTemplateForm({
  ...
  const [scope, setScope] = useState<DocumentTemplateScope>(
    template?.scope ?? "organization",
  );
  const [branchId, setBranchId] = useState(
    template?.branchId ?? (selectedBranchId === "all" ? "" : selectedBranchId),
  );
  const [status, setStatus] = useState<DocumentTemplateStatus>(
    template?.status ?? "draft",
  );
  const [outputFormat, setOutputFormat] = useState<DocumentTemplateOutput>(
    template?.outputFormat ?? "pdf",
  );
  const [variables, setVariables] = useState(template?.variables.join(", ") ?? "");
  const [selfService, setSelfService] = useState(template?.selfService ?? false);
  const [approvalRequired, setApprovalRequired] = useState(
    template?.approvalRequired ?? true,
  );
    ...
    <form onSubmit={handleSubmit} className="space-y-7">
      <section className="grid gap-5 sm:grid-cols-2">
        <FormField
          ...
          error={submitted && !title.trim() ? "Enter a template title" : undefined}
        >
          <Input
            id="templateTitle"
            value={title}
        ...
        <FormField
          ...
          error={submitted && !code.trim() ? "Enter a template code" : undefined}
        >
          <Input
            id="templateCode"
            value={code}
        ...
        <FormField label="Category" htmlFor="templateCategory">
          <Select
            id="templateCategory"
            value={category}
        ...
        <FormField label="Status" htmlFor="templateStatus">
          <Select
            id="templateStatus"
            value={status}
            onChange={(event) => setStatus(event.target.value as DocumentTemplateStatus)}
          >
            {Object.entries(DOCUMENT_TEMPLATE_STATUS_CONFIG).map(([value, config]) => (
              <option key={value} value={value}>
                {config.label}
              </option>
            ))}
        ...
        <FormField label="Scope" htmlFor="templateScope">
          <Select
            id="templateScope"
            value={scope}
            onChange={(event) => setScope(event.target.value as DocumentTemplateScope)}
          >
        ...
        <FormField label="Output format" htmlFor="templateOutput">
          <Select
            id="templateOutput"
            value={outputFormat}
            onChange={(event) =>
              setOutputFormat(event.target.value as DocumentTemplateOutput)
            }
          >
            {Object.entries(DOCUMENT_TEMPLATE_OUTPUT_CONFIG).map(([value, config]) => (
              <option key={value} value={value}>
                {config.label}
              </option>
            ))}
      ...
      <FormField label="Merge variables" htmlFor="templateVariables" optional>
        <Input
        ...
        {[
          {
            ...
            description: "Allow employees to request this generated document.",
            checked: selfService,
            onChange: setSelfService,
          },
          {
            label: "Approval required",
            description: "Require administrator approval before release.",
      ...
      <FormField label="Internal note" htmlFor="templateNote" optional>
        <Textarea
          id="templateNote"
          value={note}
