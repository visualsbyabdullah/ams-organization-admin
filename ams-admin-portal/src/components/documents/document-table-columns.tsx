import { Building2, FileText, Globe2, MoreHorizontal } from "lucide-react";
...
function actionColumn<T>({ onOpen }: OpenAction<T>): DataTableColumn<T> {
  return {
    id: "actions",
    header: DOCUMENT_COLUMN_LABELS.actions,
    headClassName: "w-16",
    className: "w-16",
    cell: (row) => (
      <Button
...
export function createDocumentColumns({
  ...
  const columns: DataTableColumn<DocumentRecord>[] = [
    {
      id: "document",
      header: DOCUMENT_COLUMN_LABELS.document,
      cell: (document) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-text-muted">
              {document.documentNumber} Â· {document.fileName}
            </p>
          </div>
        </div>
      ),
    ...
    {
      id: "owner",
      header: DOCUMENT_COLUMN_LABELS.owner,
      cell: (document) => {
        const employee = getEmployee(document.employeeId);

        if (document.ownerType === "employee" && employee) {
          return (
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-text-muted">
                  {employee.employeeCode} Â· {employee.branchName}
                </p>
              </div>
            </div>
          );
        ...
        return (
          <div className="flex items-center gap-3">
            <IconContainer
              icon={document.ownerType === "organization" ? Globe2 : Building2}
              tone="info"
            />
    ...
    {
      id: "category",
      header: DOCUMENT_COLUMN_LABELS.category,
      cell: (document) => (
    ...
    {
      id: "visibility",
      header: DOCUMENT_COLUMN_LABELS.visibility,
      cell: (document) => DOCUMENT_VISIBILITY_CONFIG[document.visibility].label,
    },
    {
      id: "uploaded",
      header: DOCUMENT_COLUMN_LABELS.uploaded,
      cell: (document) => (
        <div>
    ...
    {
      id: "expiry",
      header: DOCUMENT_COLUMN_LABELS.expiry,
      cell: (document) =>
        document.expiryDate ? formatDate(document.expiryDate) : "No expiry",
    },
    {
      id: "version",
      header: DOCUMENT_COLUMN_LABELS.version,
      cell: (document) => `v${document.version}`,
    },
    {
      id: "status",
      header: DOCUMENT_COLUMN_LABELS.status,
      cell: (document) => (
  ...
  const compactColumnIds = new Set(["document", "owner", "expiry", "status", "actions"]);
...
export function createDocumentRequestColumns({
  ...
  return [
    {
      id: "employee",
      header: DOCUMENT_COLUMN_LABELS.employee,
      cell: (request) => {
    ...
    {
      id: "request",
      header: DOCUMENT_COLUMN_LABELS.request,
      cell: (request) => (
        <div>
    ...
    {
      id: "due",
      header: DOCUMENT_COLUMN_LABELS.dueDate,
      cell: (request) => formatDate(request.dueDate),
    },
    {
      id: "mandatory",
      header: DOCUMENT_COLUMN_LABELS.mandatory,
      cell: (request) => (
        <Badge variant={request.mandatory ? "warning" : "neutral"}>
          {request.mandatory ? "Required" : "Optional"}
        </Badge>
      ),
    },
    {
...
export function createDocumentTemplateColumns({
  ...
  return [
    {
      id: "template",
      header: DOCUMENT_COLUMN_LABELS.template,
      cell: (template) => (
        <div>
    ...
    {
      id: "scope",
      header: DOCUMENT_COLUMN_LABELS.scope,
      cell: (template) => (
        <div>
          <Badge variant={DOCUMENT_TEMPLATE_SCOPE_CONFIG[template.scope].badgeVariant}>
            {DOCUMENT_TEMPLATE_SCOPE_CONFIG[template.scope].label}
          </Badge>

          <p className="mt-1 text-xs text-text-muted">
            {template.branchName ?? "All branches"}
          </p>
        </div>
      ),
    },
    {
      id: "output",
      header: DOCUMENT_COLUMN_LABELS.output,
      cell: (template) => DOCUMENT_TEMPLATE_OUTPUT_CONFIG[template.outputFormat].label,
    },
    {
      id: "controls",
      header: DOCUMENT_COLUMN_LABELS.controls,
      cell: (template) => (
        <div className="flex flex-wrap gap-2">
          <Badge variant={template.selfService ? "success" : "neutral"}>
            {template.selfService ? "Self-service" : "Admin only"}
          </Badge>
    ...
    {
      id: "updated",
      header: DOCUMENT_COLUMN_LABELS.updated,
      cell: (template) => (
        <div>
          <p className="font-medium">{formatDate(template.updatedAt)}</p>

          <p className="mt-1 text-xs text-text-muted">{template.updatedBy}</p>
        </div>
      ),
    },
    {
...
export function createDocumentSettingsColumns({
  ...
  return [
    {
      id: "settings",
      header: DOCUMENT_COLUMN_LABELS.settings,
      cell: (settings) => (
        <div>
    ...
    },
    {
      ...
      header: DOCUMENT_COLUMN_LABELS.scope,
      cell: (settings) => (
        <Badge variant={DOCUMENT_SETTINGS_SCOPE_CONFIG[settings.scope].badgeVariant}>
          {DOCUMENT_SETTINGS_SCOPE_CONFIG[settings.scope].label}
        </Badge>
      ),
    },
    {
      id: "reminders",
      header: DOCUMENT_COLUMN_LABELS.reminders,
      cell: (settings) => (
        <div>
          <p className="font-medium">{settings.expiryReminderDays} days</p>

          <p className="mt-1 text-xs text-text-muted">
            Second reminder {settings.secondExpiryReminderDays} days
          </p>
        </div>
      ),
    },
    {
      id: "upload",
      header: DOCUMENT_COLUMN_LABELS.uploadLimit,
      cell: (settings) => (
        <div>
          <p className="font-medium">{settings.maximumUploadMb} MB</p>

          <p className="mt-1 text-xs text-text-muted">
            {settings.allowedFileExtensions.length} file types
          </p>
        </div>
      ),
    },
    {
      id: "retention",
      header: "Retention",
      cell: (settings) => `${settings.retentionYears} years`,
    },
    {
      id: "status",
      header: DOCUMENT_COLUMN_LABELS.status,
