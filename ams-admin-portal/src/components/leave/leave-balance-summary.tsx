...
export function LeaveBalanceSummary({ balance, type }: LeaveBalanceSummaryProps) {
  const allowance = balance?.allowance[type] ?? 0;

  const used = balance?.used[type] ?? 0;

  const remaining = Math.max(allowance - used, 0);

  const typeConfig = LEAVE_TYPE_CONFIG[type];
  ...
  const items = [
    ...
    },
    {
      ...
      icon: CalendarClock,
      tone:
        remaining > 0 ? "bg-success-muted text-success" : "bg-danger-muted text-danger",
    },
  ];
    ...
    <section>
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-xs text-text-muted">{typeConfig.label}</p>
        </div>
      </div>
        ...
        {items.map((item) => {
          ...
          return (
            <Card key={item.label} className="p-4">
              ...
              <p className="mt-4 text-xs text-text-muted">{item.label}</p>

              <p className="mt-1 text-xl font-bold">{item.value} days</p>
            </Card>
          );
        })}
      </div>
