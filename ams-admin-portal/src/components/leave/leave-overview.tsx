...
import { LEAVE_BALANCES, LEAVE_REQUESTS, LEAVE_TRENDS } from "@/data/leave";
...
function getDaysUntil(date: string) {
  const start = new Date(`${LEAVE_REFERENCE_DATE}T00:00:00`);
...
export function LeaveOverview() {
  ...
  const scopedRequests = useMemo(
    () => requests.filter((request) => scopedEmployeeIds.has(request.employeeId)),
    [requests, scopedEmployeeIds],
  );
  ...
  const employeesOnLeave = scopedRequests.filter(
    (request) =>
      request.status === "approved" &&
      isDateWithinRange(LEAVE_REFERENCE_DATE, request.startDate, request.endDate),
  );

  const upcomingRequests = scopedRequests
    .filter((request) => {
      const daysUntil = getDaysUntil(request.startDate);

      return request.status === "approved" && daysUntil > 0 && daysUntil <= 7;
    })
    .sort((first, second) => first.startDate.localeCompare(second.startDate));

  const scopedBalances = LEAVE_BALANCES.filter((balance) =>
    scopedEmployeeIds.has(balance.employeeId),
  );

  const annualRemaining = scopedBalances.reduce(
    (total, balance) =>
      total + Math.max(balance.allowance.annual - balance.used.annual, 0),
    0,
  );

  const trend = LEAVE_TRENDS[selectedBranchId] ?? LEAVE_TRENDS.all;
  ...
  const metrics = [
    ...
    },
    {
      ...
      detail: "Starting within seven days",
      icon: CalendarClock,
      tone: "warning" as const,
    },
    {
      ...
      detail: "Combined remaining balance",
      icon: CalendarCheck2,
  ...
  return (
    <div className="mx-auto max-w-360">
      <PageHeader
        eyebrow={LEAVE_COPY.eyebrow}
        title={LEAVE_COPY.title}
        description={LEAVE_COPY.description}
        actions={
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[1.3fr_0.7fr]">
        <ChartCard
          title={LEAVE_COPY.trendTitle}
          description={LEAVE_COPY.trendDescription}
        >
        ...
        <Card className="p-6">
          <div>
            <h2 className="text-lg font-bold">{LEAVE_COPY.upcomingTitle}</h2>
          ...
          <div className="mt-5 space-y-3">
            {upcomingRequests.length > 0 ? (
                  ...
                  >
                    <div>
                      ...
                      <p className="mt-1 text-xs text-text-muted">
                        {formatDate(request.startDate)} · {request.totalDays} days
                      </p>
                    </div>
              ...
              <div className="rounded-control bg-success-muted p-4 text-sm font-medium text-success">
                No approved absences start within the next seven days.
              </div>
            )}
          </div>
        </Card>
      ...
      <Card className="mt-6">
        <div className="border-b border-border p-5">
          <h2 className="text-lg font-bold">{LEAVE_COPY.tableTitle}</h2>

          <p className="mt-1 text-sm text-text-muted">{LEAVE_COPY.tableDescription}</p>

          <SearchFilterBar
            searchValue={searchQuery}
            onSearchChange={setSearchQuery}
            searchPlaceholder={LEAVE_COPY.searchPlaceholder}
            ...
            filters={[
              {
                ...
                children: (
                  <>
                    <option value="all">{LEAVE_COPY.allTypes}</option>
              ...
              },
              {
                ...
                children: (
                  <>
                    <option value="all">{LEAVE_COPY.allStatuses}</option>
        ...
        {visibleRequests.length > 0 ? (
          ...
          <div className="flex min-h-72 flex-col items-center justify-center p-8 text-center">
            ...
            <h3 className="mt-4 font-bold">No leave requests found</h3>

            <p className="mt-2 text-sm text-text-muted">
              Change the filters or submit a new leave request.
            </p>
          </div>
        )}
      </Card>
            ...
            <section className="rounded-card border border-border">
              ...
              <DetailGrid
                ...
                items={[
                  ...
                  {
                    label: "Reviewer",
                    value: selectedRequest.reviewedBy || "Not assigned",
                  },
                  {
                    label: "Reviewed date",
