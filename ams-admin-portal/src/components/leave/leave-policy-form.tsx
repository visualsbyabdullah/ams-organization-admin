...
export function LeavePolicyForm({ policy, onCancel, onSave }: LeavePolicyFormProps) {
  const [name, setName] = useState(policy?.name ?? "");

  const [description, setDescription] = useState(policy?.description ?? "");

  const [leaveType, setLeaveType] = useState<LeaveType>(policy?.leaveType ?? "annual");

  const [status, setStatus] = useState<LeavePolicyStatus>(policy?.status ?? "draft");

  const [scope, setScope] = useState<LeavePolicyScope>(policy?.scope ?? "all_branches");
  ...
  const [annualAllowance, setAnnualAllowance] = useState(
    String(policy?.annualAllowance ?? 20),
  );

  const [accrualMethod, setAccrualMethod] = useState<LeaveAccrualMethod>(
    policy?.accrualMethod ?? "annual",
  );

  const [maximumConsecutiveDays, setMaximumConsecutiveDays] = useState(
    String(policy?.maximumConsecutiveDays ?? 10),
  );

  const [minimumNoticeDays, setMinimumNoticeDays] = useState(
    String(policy?.minimumNoticeDays ?? 7),
  );

  const [carryForwardEnabled, setCarryForwardEnabled] = useState(
    policy?.carryForwardEnabled ?? false,
  );

  const [carryForwardLimit, setCarryForwardLimit] = useState(
    String(policy?.carryForwardLimit ?? 0),
  );

  const [attachmentRequiredAfterDays, setAttachmentRequiredAfterDays] = useState(
    String(policy?.attachmentRequiredAfterDays ?? 0),
  );
  ...
  const [approvalMode, setApprovalMode] = useState<LeaveApprovalMode>(
    policy?.approvalMode ?? "manager",
  );
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    onSave({
      ...
      scope,
      branchIds,
      annualAllowance: toPositiveNumber(annualAllowance),
      accrualMethod,
      maximumConsecutiveDays: toPositiveNumber(maximumConsecutiveDays),
      minimumNoticeDays: toPositiveNumber(minimumNoticeDays),
      carryForwardEnabled,
      carryForwardLimit: carryForwardEnabled ? toPositiveNumber(carryForwardLimit) : 0,
      attachmentRequiredAfterDays: toPositiveNumber(attachmentRequiredAfterDays),
      allowNegativeBalance,
      approvalMode,
      applicableEmployees: getLeavePolicyEmployeeCount(scope, branchIds),
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-7">
      <section className="grid gap-5 md:grid-cols-2">
        ...
        <FormField label="Policy status" htmlFor="leavePolicyStatus">
          <Select
            id="leavePolicyStatus"
            value={status}
            onChange={(event) => setStatus(event.target.value as LeavePolicyStatus)}
          >
        ...
        <FormField label="Leave type" htmlFor="policyLeaveType">
          <Select
            id="policyLeaveType"
            value={leaveType}
        ...
        >
          <Input
            ...
            onChange={(event) => setAnnualAllowance(event.target.value)}
          />
        </FormField>
      ...
      <section>
        ...
        <p className="mt-1 text-sm text-text-muted">
          Choose where this leave policy should apply.
        </p>
          ...
          {[
            {
              ...
              label: "All branches",
              description: "Apply across the complete organization.",
            },
            {
              value: "selected_branches" as const,
              label: "Selected branches",
              description: "Restrict policy to chosen branches.",
            },
          ].map((option) => (
            <button
              key={option.value}
        ...
        {scope === "selected_branches" && (
          <div className="mt-4">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {LEAVE_POLICY_BRANCH_OPTIONS.map((branch) => {
      ...
      <section>
        <h3 className="font-bold">Entitlement rules</h3>

        <div className="mt-5 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
          <FormField label="Accrual method" htmlFor="leaveAccrualMethod">
            <Select
              id="leaveAccrualMethod"
              value={accrualMethod}
              onChange={(event) =>
                setAccrualMethod(event.target.value as LeaveAccrualMethod)
              }
            >
              {Object.entries(LEAVE_POLICY_ACCRUAL_CONFIG).map(([value, config]) => (
                <option key={value} value={value}>
                  {config.label}
                </option>
              ))}
          ...
          <FormField label="Maximum consecutive days" htmlFor="maximumConsecutiveDays">
            <Input
              id="maximumConsecutiveDays"
              type="number"
              min="0"
              value={maximumConsecutiveDays}
              onChange={(event) => setMaximumConsecutiveDays(event.target.value)}
            />
          </FormField>
          ...
          >
            <Input
              ...
              onChange={(event) => setMinimumNoticeDays(event.target.value)}
            />
          </FormField>
          ...
          >
            <Input
              ...
              type="number"
              min="0"
              value={attachmentRequiredAfterDays}
              onChange={(event) => setAttachmentRequiredAfterDays(event.target.value)}
            />
          </FormField>

          <FormField label="Approval workflow" htmlFor="leaveApprovalMode">
            <Select
              id="leaveApprovalMode"
              value={approvalMode}
              onChange={(event) =>
                setApprovalMode(event.target.value as LeaveApprovalMode)
              }
            >
              {Object.entries(LEAVE_POLICY_APPROVAL_CONFIG).map(([value, config]) => (
                <option key={value} value={value}>
                  {config.label}
                </option>
              ))}
      ...
      <section>
        <h3 className="font-bold">Balance controls</h3>

        <div className="mt-4 space-y-3">
          <div className="flex items-center justify-between gap-5 rounded-control border border-border p-4">
            <div>
              <p className="text-sm font-semibold">Carry forward unused leave</p>

              <p className="mt-1 text-xs text-text-muted">
                Allow remaining leave to move into the next leave year.
              </p>
            </div>
          ...
          {carryForwardEnabled && (
            ...
            >
              <Input
                ...
                onChange={(event) => setCarryForwardLimit(event.target.value)}
              />
            </FormField>
          )}
          ...
          <div className="flex items-center justify-between gap-5 rounded-control border border-border p-4">
            <div>
              <p className="text-sm font-semibold">Allow negative leave balance</p>

              <p className="mt-1 text-xs text-text-muted">
                Employees may request more leave than their current available balance.
              </p>
            </div>
