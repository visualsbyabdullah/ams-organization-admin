...
function buildCalendarCells(month: Date) {
  ...
  const leadingCells = firstWeekday === 0 ? 6 : firstWeekday - 1;
  ...
  for (let day = 1; day <= daysInMonth; day += 1) {
    cells.push(toDateString(new Date(year, monthIndex, day)));
  }
...
function requestIncludesDate(request: LeaveRequest, date: string) {
      ...
      <div className="min-w-210">
        ...
        <div className="grid grid-cols-7">
          {cells.map((date, cellIndex) => {
            ...
            const remainingCount = dayRequests.length - visibleRequests.length;
              ...
              >
                ...
                <div className="mt-2 space-y-1.5">
                  {visibleRequests.map((request) => {
                    ...
                    const tone = LEAVE_CALENDAR_TYPE_TONES[request.type];
