...
import { createBranchColumns } from "@/components/branches/branch-table-columns";
...
import { getBranchCapacityUtilization } from "@/lib/branches";
import type { BranchRecord, BranchStatus } from "@/types/branch";
...
export function BranchesWorkspace() {
  ...
  const [branches, setBranches] = useState<BranchRecord[]>(BRANCHES);
  ...
  const scopedBranches = useMemo(
    () =>
      branches.filter(
        (branch) => selectedBranchId === "all" || branch.id === selectedBranchId,
      ),
    [branches, selectedBranchId],
  );

  const cities = useMemo(
    () => Array.from(new Set(scopedBranches.map((branch) => branch.city))).sort(),
    [scopedBranches],
  );
  ...
  const totalEmployees = scopedBranches.reduce(
    (total, branch) => total + branch.employeeCount,
    0,
  );
  const totalCapacity = scopedBranches.reduce(
  ...
  const metrics = [
    {
      label: BRANCH_COPY.metrics.total,
    ...
    {
      label: BRANCH_COPY.metrics.active,
    ...
    {
      label: BRANCH_COPY.metrics.employees,
      value: totalEmployees.toLocaleString("en-PK"),
      detail: `${totalCapacity.toLocaleString("en-PK")} total capacity`,
      icon: Users,
      tone: "warning" as const,
    },
    {
      label: BRANCH_COPY.metrics.utilization,
      value: `${utilization}%`,
      detail: "Current organization capacity",
      icon: MapPinned,
      tone: utilization >= 90 ? ("danger" as const) : ("info" as const),
    },
  ];
  ...
  function saveBranch(branch: BranchRecord) {
    setBranches((currentBranches) => {
      const exists = currentBranches.some((item) => item.id === branch.id);

      return exists
        ? currentBranches.map((item) => (item.id === branch.id ? branch : item))
  ...
  return (
    <div className="mx-auto max-w-360">
      <PageHeader
        ...
        description={BRANCH_COPY.description}
        actions={
          <Button
            onClick={() => {
      ...
      <section className="mt-6">
        <div>
          <h2 className="text-lg font-bold">{BRANCH_COPY.mapsTitle}</h2>
          <p className="mt-1 text-sm text-text-muted">{BRANCH_COPY.mapsDescription}</p>
        </div>

        <div className="mt-5 grid gap-5 xl:grid-cols-2">
          {visibleBranches.map((branch) => (
            <Card key={branch.id} className="overflow-hidden">
              <BranchMap branch={branch} compact />
              ...
              >
                ...
                <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                  <div className="rounded-control bg-canvas p-3">
                    ...
                    <p className="mt-1 font-bold">{branch.employeeCount}</p>
                  </div>

                  <div className="rounded-control bg-canvas p-3">
                    <p className="text-xs text-text-muted">Capacity</p>
                    <p className="mt-1 font-bold">
                      {getBranchCapacityUtilization(branch)}%
                    </p>
                  </div>
                </div>
              </button>
      ...
      <Card className="mt-6 overflow-hidden">
        <div className="border-b border-border p-5">
          <h2 className="text-lg font-bold">{BRANCH_COPY.registerTitle}</h2>
          <p className="mt-1 text-sm text-text-muted">
        ...
        <DataTable
          ...
          emptyState={
            <div className="flex min-h-72 flex-col items-center justify-center p-8 text-center">
              ...
              <h3 className="mt-4 font-bold">{BRANCH_COPY.emptyTitle}</h3>
              <p className="mt-2 text-sm text-text-muted">
                {BRANCH_COPY.emptyDescription}
              </p>
            </div>
          }
        />
          ...
          selectedRecord ? (
            <div className="flex flex-wrap justify-end gap-3">
              {selectedRecord.status === "active" ? (
                <Button variant="outline" onClick={() => updateBranchStatus("inactive")}>
                  {BRANCH_COPY.actions.deactivate}
                </Button>
              ) : (
                <Button variant="outline" onClick={() => updateBranchStatus("active")}>
                  {BRANCH_COPY.actions.activate}
                </Button>
              )}
      ...
      >
        {selectedRecord && <BranchDetails branch={selectedRecord} />}
      </Drawer>

      <Drawer
        open={editorMode !== null}
        onClose={() => setEditorMode(null)}
        title={editorMode === "create" ? "Add branch" : "Edit branch"}
        description="Configure branch identity, location coordinates, capacity and operational controls."
      >
        {editorMode && (
          <BranchForm
            key={editorMode === "create" ? "new-branch" : selectedRecord?.id}
            branch={editorMode === "edit" ? (selectedRecord ?? undefined) : undefined}
