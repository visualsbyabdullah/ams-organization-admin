...
import { BRANCH_COPY } from "@/config/branches";
import {
...
export function BranchMap({ branch, compact = false }: BranchMapProps) {
  return (
    <div className="overflow-hidden rounded-card border border-border bg-surface">
      <div className={compact ? "h-44" : "h-72"}>
        <iframe
          title={`${branch.name} map`}
          src={getOpenStreetMapEmbedUrl(branch)}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">{getBranchCoordinates(branch)}</span>
        </div>

        <a
          href={getOpenStreetMapPageUrl(branch)}
          target="_blank"
          rel="noreferrer"
