...
export function getBranchCapacityUtilization(
  branch: BranchRecord,
) {
...
export function getBranchCoordinates(
  ...
  return `${branch.latitude.toFixed(
    4,
...
export function getOpenStreetMapEmbedUrl(
  ...
  const minimumLongitude =
    branch.longitude - longitudeDelta;
  const maximumLongitude =
    branch.longitude + longitudeDelta;
  const minimumLatitude =
    branch.latitude - latitudeDelta;
...
export function formatBranchWorkingDays(
  branch: BranchRecord,
) {
  return branch.workingDays
    .map(
      (day) =>
        BRANCH_WORKING_DAY_CONFIG[day]
          .shortLabel,
    )
    .join(", ");
}
