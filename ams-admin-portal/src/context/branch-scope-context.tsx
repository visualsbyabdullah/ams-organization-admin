...
export function BranchScopeProvider({ children }: BranchScopeProviderProps) {
  const [selectedBranchId, setSelectedBranchId] = useState(BRANCH_OPTIONS[0].id);

  const selectedBranch =
    BRANCH_OPTIONS.find((branch) => branch.id === selectedBranchId) ?? BRANCH_OPTIONS[0];
  ...
  return (
    <BranchScopeContext.Provider value={value}>{children}</BranchScopeContext.Provider>
  );
}
